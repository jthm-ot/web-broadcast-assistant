...
typedef struct source_data {
	...
	uint32_t interval;     /* Periodic advertising interval (N * 1.25 ms) */
...
static void reset_source_data(uint8_t pa_attempt)
{
	...
	for (int i = 0; i < MAX_NUMBER_OF_SOURCES; i++) {
		...
		source_data_list.data[source_data_list.num].sid = 0;
		source_data_list.data[source_data_list.num].interval = 0;
...
static source_data_t *source_data_add(const bt_addr_le_t *addr, uint8_t sid, uint32_t interval)
{
	...
	if (source_data_list.num < MAX_NUMBER_OF_SOURCES) {
		...
		source_data_list.data[source_data_list.num].sid = sid;
		source_data_list.data[source_data_list.num].interval = interval;
	...
	if (sr_data->broadcast_id != INVALID_BROADCAST_ID) {
		...
		if (!pa_syncing) {
			...
			if (source_data != NULL && source_data->pa_attempt_cd > 0) {
				...
				int err = pa_sync_create(&source_data->addr, source_data->sid,
							 source_data->interval);
				if (err != 0) {
...
int broadcast_assistant_pa_sync(bt_addr_le_t *addr)
{
	...
	err = pa_sync_create(&source_data->addr, source_data->sid, source_data->interval);
	if (err != 0) {
